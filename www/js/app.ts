...
angular.module('starter', ['ionic', 'LocalStorageModule', 'starter.controllers', 'starter.services', 'ngResource', 'ngCordova'])
  ...
  .run(function ($ionicPlatform: ionic.platform.IonicPlatformService,
    $state: ng.ui.IStateService, $ionicHistory: ionic.navigation.IonicHistoryService, $location: ng.ILocationService,
    $ionicPopup: ionic.popup.IonicPopupService, $timeout: ng.ITimeoutService, $http: ng.IHttpService,
    $cordovaAppVersion: any, appConfig: IAppConfig, $rootScope: IParkingRootScope,
    localStorageService: ng.local.storage.ILocalStorageService, $cordovaSplashscreen: any,
    RegisterService: RegisterService, UpdateService: UpdateService) {
    $ionicPlatform.ready(function () {
      ...
      //主页面显示退出提示框
      $ionicPlatform.registerBackButtonAction(function (e: any) {
      ...
      var u: User = <User>localStorageService.get('currentUser');
      if (u) {
        $rootScope.currentUser = u;
        //从服务器更新用户数据
        RegisterService.getUserByPhoneNumber(u.phoneNumber).then((user: User) => {
      ...
      if (isIPad || isIOS || isAndroid) {
        ...
        UpdateService.checkNewVersion().then(newVersion => {
          if (newVersion) UpdateService.install();
        });
      }
      // $cordovaSplashScreen.hide();
  ...
  .service('AuthInterceptor', function ($rootScope: IParkingRootScope) {
    ...
    service.responseError = function (response: any) {
  ...
  .config(function ($stateProvider: ng.ui.IStateProvider, $httpProvider: ng.IHttpProvider,
    $urlRouterProvider: ng.ui.IUrlRouterProvider, $ionicConfigProvider: any) {
