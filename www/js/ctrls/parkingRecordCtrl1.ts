interface IParkingRecordScope extends ng.IScope {
    records: CarState[];
    pageNumber: number;
    loadMore: any;
    more: boolean;
...
angular.module('starter.controllers').controller('ParkingRecordCtrl1',
    function ($scope: IParkingRecordScope, $rootScope: IParkingRootScope, $timeout: ng.ITimeoutService, $ionicPopup: ionic.popup.IonicPopupService, $ionicLoading: ionic.loading.IonicLoadingService, ParkingService: ParkingService) {
        ...
        $scope.loadMore = function () {
            ...
            ParkingService.getRecordsByPageNo($scope.pageNumber, 10, plates)
                .then(recs => {
                    ...
                    //$scope.more = true;
                    if (recs.length < 10 ) $scope.hasMoreData = false;
                });
